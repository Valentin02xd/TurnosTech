...
def init_database():
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS appointments (
            ...
            date TEXT NOT NULL,
            time TEXT NOT NULL,
            room TEXT NOT NULL DEFAULT 'informatica',
    ...
    try:
        cursor.execute("SELECT room FROM appointments LIMIT 1")
    except sqlite3.OperationalError:
        print("Agregando columna 'room' a la tabla appointments...")
        cursor.execute("ALTER TABLE appointments ADD COLUMN room TEXT NOT NULL DEFAULT 'informatica'")
        print("Columna 'room' agregada exitosamente")
    ...
    def handle_create_appointment(self):
        try:
            ...
            with db_lock:
                ...
                room = data.get('room', 'informatica')
                
                cursor.execute(
                    'INSERT INTO appointments (user_id, name, email, date, time, room) VALUES (?, ?, ?, ?, ?, ?)',
                    (user['id'], user['name'], user['email'], data['date'], data['time'], room)
                )
        ...
        try:
            ...
            with db_lock:
                ...
                if room == 'all':
                    ...
                    cursor.execute('SELECT * FROM appointments WHERE room = ? ORDER BY date, time', (room,))
